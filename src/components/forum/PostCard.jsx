      ...
      <div className={`glass-card border-l-[3px] ${getTopicColor(post.topic)} ${post.isPinned ? 'ring-1 ring-primary/20' : ''} p-5 mb-4`}>
         <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-2">
               ...
               {post.language && (
                  <span className="text-[10px] text-text-secondary/70 font-medium px-1.5 py-0.5 glass rounded border border-accent-gold/10 uppercase tracking-tight">
                     {post.language === 'en' ? '🇬🇧 EN' : `🇮🇳 ${post.language.toUpperCase()}`}
                  </span>
               )}
               <span className="text-xs text-text-secondary font-medium px-2 py-1 glass rounded-md border border-accent-gold/10">
                  {post.topic}
               </span>
               {post.type === 'question' && <span className="text-xs bg-accent-gold/10 text-accent-gold px-2 py-0.5 rounded-full">Question</span>}
               {post.isExpertAnswered && <span className="text-xs glass text-text-secondary px-2 py-0.5 rounded-full border border-accent-gold/10">Expert Replied</span>}
            </div>
         ...
         >
            <h3 className="text-ivory text-base font-bold mb-2 leading-snug group-hover:text-accent-gold transition-colors line-clamp-2">
