export default function SymptomCard({ condition }) {
  const getProbabilityColor = (probability) => {
    ...
    return 'bg-accent-gold/5 text-accent-gold border border-accent-gold/10';
  };

  return (
    <div className="glass-card p-6 mb-4 group hover:shadow-glass-hover transition-all">
      <div className="flex justify-between items-start gap-4">
        <div className="flex-1">
          <h3 className="text-xl font-bold text-ivory mb-2 group-hover:text-accent-gold transition-colors">{condition.name}</h3>
